...
import type { IRokuFeed, IRokuEpisode, IVideo } from "../../../src/types";
...
export default function feed(req: NextApiRequest, res: NextApiResponse) {
  ...
  for (var i = 0; i < s00.length; i++) {
    allMetadata[0].push(JSON.parse(fs.readFileSync(`${config.metadataPath}/00/${s00[i]}`, "utf-8")));
  }

  const s01 = fs.readdirSync(`${config.metadataPath}/01`);
  for (var i = 0; i < s01.length; i++) {
    allMetadata[1].push(JSON.parse(fs.readFileSync(`${config.metadataPath}/01/${s01[i]}`, "utf-8")));
  }
    ...
    series: [
      {
        ...
        shortDescription:
          "What would you do if you only had a year left to live? Would you squander the time you were given? Or would you make every second count? Welcome to Unus Annus. In exactly 365 days this channel will be...",
        longDescription:
  ...
  function genSeason(season: 0 | 1) {
    return allMetadata[season].map((episode: IVideo) => {
      return <IRokuEpisode>{
        ...
        content: {
          ...
          videos: episode.video
            ? [
                {
                  url: `https://${episode.video}`,
                  quality: "FHD",
                  videoType: episode.video.split(".")[episode.video.split(".").length - 1].toUpperCase(),
                },
              ]
        ...
        shortDescription:
          episode.description === ""
            ? 'This episode doesn"t have a description.'
            : episode.description.length > 197
            ? `${episode.description.substr(0, 197)}...`
            : episode.description,
