...
export default function sendEmail(type:string, to:string, replaceFunction:(string:string, isHTML:boolean) => string = (string:string) => ***REMOVED*** return string; ***REMOVED***):Promise<nodemailer.SentMessageInfo> ***REMOVED***
  return new Promise((resolve, reject) => ***REMOVED***
    if (emails[type]) ***REMOVED***
      const email:Email = emails[type];
      mailer.sendMail(***REMOVED***
        from: noReplyEmail,
        to,
