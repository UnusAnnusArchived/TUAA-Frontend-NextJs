...
export const YouTubeSource = z.object(
  {
    ...
    name: z.string({ errorMap }),
    id: z.string({ errorMap }),
    youtubeId: z.string({ errorMap }).regex(/^[\w\d_\-]{11}$/),
  },
  { errorMap }
);

export const Source = z.union([BunnySource, DirectSource, YouTubeSource], { errorMap });
